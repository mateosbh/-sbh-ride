...
export default function PlacePicker({label,value,places,onChange,type}:{label:string;value?:Place;places:Place[];onChange:(p:Place)=>void;type:'from'|'to'}){
 ...
 const local=useMemo(()=>{const q=query.toLowerCase().trim();return q?[...catalog,...places].filter(p=>(p.name+' '+p.description+' '+p.category).toLowerCase().includes(q)).slice(0,30):[...places,...catalog].slice(0,30)},[places,catalog,query])
 const results=useMemo(()=>{const seen=new Set<string>();return[...online,...local].filter(p=>{const k=p.name.toLowerCase()+'|'+p.lat.toFixed(4)+'|'+p.lng.toFixed(4);if(seen.has(k))return false;seen.add(k);return true}).slice(0,40)},[local,online])
