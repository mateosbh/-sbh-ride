...
export default function MapView({pickup,destination,driverProgress,onMovePickup,onMoveDestination,userLocation,compact=false}:{pickup?:Place;destination?:Place;driverProgress?:number;onMovePickup?:(lat:number,lng:number)=>void;onMoveDestination?:(lat:number,lng:number)=>void;userLocation?:{lat:number;lng:number};compact?:boolean}){
 ...
 return <div className={`map ${compact?'compact':''}`}>
  <MapContainer center={[17.905,-62.835]} zoom={13} minZoom={13} maxZoom={25} maxBounds={SBH_BOUNDS} maxBoundsViscosity={1} inertia={false} worldCopyJump={false} zoomControl={false} attributionControl={false}>
   {mapStyle==='satellite'?<TileLayer attribution='Tiles &copy; Esri &mdash; Source: Esri, Maxar, Earthstar Geographics, and the GIS User Community' url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}" maxNativeZoom={17} maxZoom={25}/>:<TileLayer attribution='&copy; OpenStreetMap contributors' url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" maxNativeZoom={17} maxZoom={25}/>}
