...
## Mise en ligne HTTPS avec GitHub Pages

Le workflow `.github/workflows/deploy-pages.yml` construit et publie automatiquement
le site à chaque push sur `main`. Après fusion de ce workflow :

1. Ouvrir **Settings → Pages** dans le dépôt GitHub.
2. Dans **Build and deployment → Source**, choisir **GitHub Actions**.
3. Ouvrir l'onglet **Actions**, puis lancer **Deploy SBH Ride to GitHub Pages**
   avec **Run workflow** si le déploiement ne démarre pas automatiquement.
4. L'URL HTTPS est affichée dans le résumé du job `build-and-deploy` et dans
   **Settings → Pages**. Elle suit généralement la forme
   `https://<compte>.github.io/<depot>/`.

La configuration Vite utilise des chemins relatifs afin que les ressources
fonctionnent aussi bien sur cette URL de projet que sur un futur domaine personnalisé.
